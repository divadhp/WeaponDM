package com.example.weapondm.model
...
class Inference(file: File, labels: Vector<String>) {
    ...
    fun inference(bitmap: Bitmap) : Array<Prediction> {
        ...
        val reconocimientos = Array(numDetections[0].toInt()) { i ->
            Prediction(
                labels[outputClasses[0][i].toInt() + 1],
                outputScores[0][i],
                RectF(
                    outputLocations[0][i][1] * previewX,
                    outputLocations[0][i][0] * previewY,
                    outputLocations[0][i][3] * previewX,
                    outputLocations[0][i][2] * previewY
                )
            )
        }
